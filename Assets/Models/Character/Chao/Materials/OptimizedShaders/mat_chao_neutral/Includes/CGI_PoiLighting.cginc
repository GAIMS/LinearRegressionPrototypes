...
void calculateBasePassLightMaps()
{
        ...
        switch(float(0))
        {
            case 0: // Ramp Texture
            {
                poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + float(-0.006)).rgb, shadowStrength.r);
            ...
            {
                poiLight.rampedLightMap = saturate(1 - smoothstep(float(0) - .000001, float(0.503), 1 - poiLight.lightMap));
        ...
        if (float(0) == 2) // Wrapped
        {
            ...
            uint normalsindex = float(0) > 0 ? 1: 0;
                    ...
                    {
                        poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, float(0)), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.503), 1 - poiLight.lightMap)));
        ...
        float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
        {
            ...
            {
                return lerp(lightColor * attenuation, lightColor * float(0.5) * attenuation, smoothstep(float(0), float(0.179), dotNL)) * detailShadow;
            }
            else //if(float(0) == 2) // Wrapped
            {
                ...
                poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.503), 1 - uv)));
        ...
        void applyShadeMaps(inout float4 albedo)
        {
            ...
            if (float(0) == 2)
            {
                ...
                float MainColorFeatherStep = float(0.01) - float(0.0001);
                ...
                float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.01) - MainColorFeatherStep) * (shadowMask));
        ...
        float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
        {
            float3 finalLighting = 1;
                ...
                else if (float(0) == 1) // Toon
                {
                        ...
                        finalLighting = lerp(poiLight.color * max(poiLight.additiveShadow, float(0.5)), poiLight.color * float(0.5), smoothstep(float(0), float(0.179), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation * detailShadow;
                    #else
                        finalLighting = lerp(poiLight.color * max(poiLight.attenuation, float(0.5)), poiLight.color * float(0.5), smoothstep(float(0), float(0.179), 1 - (.5 * poiLight.nDotL + .5))) * detailShadow;
                    #endif
                }
                else //if(float(0) == 2) // Wrapped
                {
